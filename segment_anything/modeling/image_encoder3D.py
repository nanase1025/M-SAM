...
class ImageEncoderViT3D(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            block = Block3D(
                ...
                window_size=window_size if i not in global_attn_indexes else 0, # 14
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.patch_embed(x) # [batch, 8, 8, 8, 768]
        ...
        for blk in self.blocks:
            x = blk(x) # [1,16,16,16,768]
        x = self.neck(x.permute(0, 4, 1, 2, 3)) # [2, 384, 8, 8, 8]
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x).reshape(B, D * H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4) # [3, 2, 12, 512, 64]

            
        # q, k, v with shape (B * nHead, H * W, C)
        q, k, v = qkv.reshape(3, B * self.num_heads, D * H * W, -1).unbind(0) # [24, 512, 64]
